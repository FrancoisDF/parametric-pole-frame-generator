...
import { numSectionsPerSide, calculateSections, type Section } from './sectioning.js';
...
export function generateCombinedSVGPlan(params: Params): string {
  ...
  for (let c = 0; c < ns - 1; c++) colOffsets.push(colOffsets[c] + colWidths[c] + gap);
  const rowOffsets: number[] = [0];
  for (let r = 0; r < ns - 1; r++) rowOffsets.push(rowOffsets[r] + rowHeights[r] + gap);

  const totalW = colOffsets[ns - 1] + colWidths[ns - 1];
  const totalH = rowOffsets[ns - 1] + rowHeights[ns - 1];
